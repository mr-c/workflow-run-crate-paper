...
Thus, run details from all the executions are represented in the retrospective provenance RO-Crate. Support for Workflow Run RO-Crate is available since WfExS-backend version 0.10.1 \cite{Fernandez 2023a}.
...
An example of Workflow Run Crate generated by WfExS-backend from a Nextflow workflow run \cite{Bouyssie 2023} is available from Zenodo \cite{Fernandez 2023b}.
...
\begin{table}[!ht]
  ...
  WfExS & Workflow & \footnotesize \cite{Fernandez 2023a} & \footnotesize \cite{Fernandez 2023b} \\
...
\end{verbatim}
\caption{\label{lst:ml_pipeline_streamflow_report}\texttt{runcrate report} command line output. This informal listing of relevant RO-Crate entities describe each step execution. Note that inputs and outputs are of different types (not shown), e.g. \texttt{tissue\_low>0.9} is a string parameter, \texttt{6b15de\ldots} is a filename, and \texttt{\#af0253\ldots} is a collection.}
\end{footnotesize}
...
\ref{analysis_table}.
The three dots (\ldots) in the WRROC column indicate that  the concept is supported in an RO-Crate using existing schema.org vocabulary (e.g. \url{https://schema.org/softwareHelp}) but is not required or recommended by the WRROC profiles.
